...
async def get_user_history(request):
    ...
    try:
        ...
        if item_type in ('all', 'translation'):
            tr_result = supabase.table('translations').select('*').eq('user_id', user_id).order('created_at', desc=True).range(offset, offset + limit - 1).execute()
            translations = [
...
async def get_usage_details(request):
    ...
    try:
        ...
        # Transcription usage
        t_usage_result = supabase.table('transcription_usage').select('*').eq('user_id', user_id).gte('created_at', since).order('created_at', desc=True).execute()
        t_usage = t_usage_result.data or []

        # Translation usage
        tr_usage_result = supabase.table('translation_usage').select('*').eq('user_id', user_id).gte('created_at', since).order('created_at', desc=True).execute()
